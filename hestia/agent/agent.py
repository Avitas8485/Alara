...
class Agent(metaclass=Singleton):
    def __init__(self):
        ...
        self.agent_state = self.automation_handler.state_machine.add_state(State(entity_id=self.agent_name, state="idle", attributes={"last_interaction": None}))
    ...
    def process_user_prompt(self, user_prompt):
        ...
        self.automation_handler.state_machine.set_state(entity_id=self.agent_name, new_state="processing", attributes={"last_interaction": self.last_interaction})
    ...
    def handle_wakeword(self):
        ...
        self.automation_handler.state_machine.set_state(entity_id=self.agent_name, new_state="listening", attributes={"last_interaction": self.last_interaction})
    ...
    def run(self):
        while self.running:
            if self.last_interaction is not None and time.time() - self.last_interaction > 10:
                ...
                self.automation_handler.state_machine.set_state(entity_id=self.agent_name, new_state="idle", attributes={"last_interaction": None})
                self.last_interaction = None
